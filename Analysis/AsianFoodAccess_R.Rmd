...
options(digits = 4)
options(scipen=4)
FullDataset <- read_excel("FullDataset_Final.xlsx", 
...
#Mean and standard deviation of Asian American and Pacific Islander (AAPI)
#Diabetes Rate of all San Diego Census Tracts (CTs)
#Unit: number of hospitalizations and emergency department discharges for 
#diabetes per 100,000 AAPI population per year
FullDataset %>%
...
#Mean and standard deviation of Percent below 200% Poverty Line of all 
#San Diego CTs
FullDataset %>%
...
#Mean and standard deviation of Percent of Population over 65 of all San Diego 
#CTs
FullDataset %>%
...
#Mean and standard deviation of Percent of Population who are obese of all 
#San Diego CTs
FullDataset %>%
...
#Mean and standard deviation of percent of CT population >½ mile to the 
#nearest supermarket of the 564 out of 626 San Diego CTs with data available
FullDataset %>%
...
#Mean and standard deviation of percent of CT’s AAPI population >½ mile 
#to the nearest supermarket of the 564 out of 626 San Diego CTs with 
#data available
FullDataset %>%
...
#Mean and standard deviation of the Yelp ratio of Asian restaurants to 
#non-Asian restaurants of all San Diego CTs 
FullDataset %>%
...
#Mean and standard deviation of the Yelp ratio of Asian groceries to 
#non-Asian groceries of all San Diego CTs 
FullDataset %>%
...
#Mean and standard deviation of the Yelp ratio of Asian restaurants to 
#AAPI population of all San Diego CTs 
FullDataset %>%
...
#Mean and standard deviation of the Yelp proportion of CT Asian restaurants 
#to the total number of Asian restaurants in San Diego of all San Diego CTs
FullDataset %>%
...
#Mean and standard deviation of the Yelp proportion of CT Asian groceries 
#to the total number of Asian groceries in San Diego of all San Diego CTs
FullDataset %>%
...
#A subset of variables were selected from the FullDataset to run a correlation
#matrix among the covariates
...
#A red filling in the cell indicated a positive correlation, and a blue filling 
#indicated a negative correlation. The P value for each correlation was also
...
#A subset of variables were selected from the FullDataset to run a correlation
#matrix among the food access variables
...
#A red filling in the cell indicated a positive correlation, and a blue filling 
#indicated a negative correlation. The P value for each correlation was also
#calculated (shown in the p matrix).Correlations with p>.05 were crossed out 
#in the matrix.
...
```{r FARA_AAPI OLS}
#Food Access Variable: percent of CT’s AAPI population >½ mile to the nearest 
#supermarket
...
#Residuals of the OLS model were exported to a new column in FullDataset,
#"FARA_AAPI"
...
```{r AR_NAR OLS}
#Food Access Variable: the Yelp ratio of CT Asian restaurants to CT non-Asian 
#restaurants
...
```{r AG_NAG OLS}
#Food Access Variable: the Yelp ratio of CT Asian groceries to CT non-Asian 
#groceries
...
```{r AR_AAPI OLS}
#Food Access Variable: the Yelp ratio of CT Asian restaurants to CT AAPI 
#population
...
```{r AG_AAPI OLS}
#Food Access Variable: the Yelp ratio of CT Asian groceries to CT AAPI 
#population
...
```{r ctAG_sdAG OLS}
#Food Access Variable: the Yelp proportion of CT Asian grocery stores to the 
#total number of Asian grocery stores in San Diego County
...
# 4. Export the New FullDataset with OLS Residuals as "FullDatasetNew.xlsx"

To test the spatial autocorrelation of OLS residuals, the new FullDataset with
OLS residual columns was exported as "FullDatasetNew.xlsx" and imported
into ArcGIS for spatial autocorrelation analyses.

```{r}
library(rio)
install_formats()
export(FullDataset, "FullDatasetNew.xlsx")
```

# 5. Spatial Lag Regression

To prepare the dataset for spatial lag regression, the "FullDatasetNew" file 
was first imported to ArcGIS to generate a shapefile for the spatial lag 
...
library(spdep)
library(maptools)
SpatialDataset <- readShapePoly("SpatialDataset.shp")
...
#Residuals of the SLR model were exported to a new column in FullDataset,
#"FARA_AAPI_SLR"
...
# 6. Export the New FullDataset with SLR Residuals as "FullDatasetSpatial.xlsx"

To test the spatial autocorrelation of SLR residuals, the new FullDataset with
SLR residual columns was exported as "FullDatasetSpatial.xlsx" and imported
into ArcGIS for spatial autocorrelation analyses.

```{r FullDatasetSpatial}
library(rio)
install_formats()
export(FullDataset, "FullDatasetSpatial.xlsx")
```
